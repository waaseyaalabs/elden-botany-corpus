...
from pipelines.io.carian_fmg_loader import load_carian_item_fmg
...
CONFLICT_COLUMNS = ["name", "category", "weight", "rarity"]

DEFAULT_OUTPUT = Path("data/curated/items_canonical.parquet")

SOURCE_LOADERS: list[tuple[str, SourceLoader]] = [
    ("kaggle_dlc", load_kaggle_dlc_items),
    ("kaggle_base", load_kaggle_base_items),
    ("github_api", load_github_api_items),
    ("carian_fmg", load_carian_item_fmg),
]


def build_items_canonical(
    *,
    raw_root: Path,
    ...
    """Build the canonical Elden Ring items dataset."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build canonical Elden Ring items dataset",
    )
    parser.add_argument(
        "--raw-root",
        type=Path,
...
def main() -> None:
    ...
    try:
        ...
        LOGGER.error("Canonical items pipeline failed: %s", exc)
