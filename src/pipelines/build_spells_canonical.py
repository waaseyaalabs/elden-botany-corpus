...
from pipelines.io.carian_fmg_loader import load_carian_spell_fmg
...
REQUIRED_INT_COLUMNS = ["fp_cost", "slots_required"]
OPTIONAL_INT_DEFAULTS = {
    "stamina_cost": 0,
    "required_int": 0,
    "required_fai": 0,
    "required_arc": 0,
}
CONFLICT_COLUMNS = ["name", "spell_type", "fp_cost"]

DEFAULT_OUTPUT = Path("data/curated/spells_canonical.parquet")

SOURCE_LOADERS: list[tuple[str, SourceLoader]] = [
    ("kaggle_dlc", load_kaggle_dlc_spells),
    ("kaggle_base", load_kaggle_base_spells),
    ("github_api", load_github_api_spells),
    ("carian_fmg", load_carian_spell_fmg),
]


def build_spells_canonical(
    *,
    raw_root: Path,
    ...
    """Build the canonical dataset for sorceries and incantations."""
...
def _records_to_dataframe(records: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    normalized_types = raw_spell_types.astype(str).str.lower()
    normalized_types = normalized_types.replace(
        {
            "": "incantation",
            "none": "incantation",
            "sorceries": "sorcery",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build canonical Elden Ring spells dataset",
    )
    parser.add_argument(
        "--raw-root",
        type=Path,
...
def main() -> None:
    ...
    try:
        ...
        LOGGER.error("Canonical spells pipeline failed: %s", exc)
