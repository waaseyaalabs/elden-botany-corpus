...
class PipelineConfig(BaseModel):
    ...
    fetch_impalers: bool = Field(
        default=True, description="Fetch Impalers text"
    )

    reconcile_threshold: float = Field(
        default=0.86, description="Fuzzy match threshold"
    )
    ...
    export_by_type: bool = Field(
        default=True, description="Export separate files per type"
    )
