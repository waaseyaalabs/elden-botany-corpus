...
- **Entity Aliasing**: Carian FMG dialogue emits synthetic speaker IDs such as `npc:carian_speaker_106210010`. These IDs are mapped to canonical NPC slugs through `data/reference/entity_aliases.csv`, which supports wildcard `raw_id` patterns, confidence scores, and maintenance comments. During `build_lore_corpus` every lore row stores both the original `raw_canonical_id` and the normalized `canonical_id`; when an alias matches, the pipeline rewrites the canonical slug and recomputes the `lore_id`. Downstream motif graphs, summarizers, and guardrails therefore reason about `npc:ranni`, `npc:gideon_ofnir`, `npc:hyetta`, etc., instead of thousands of synthetic Carian speakers. Extending coverage is as simple as appending a new row to the alias CSV and rerunning the corpus build.
