        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                INSERT INTO elden.corpus_document (
                    id, source_type, source_uri, title
                )
                VALUES (
                    %s, 'kaggle_base',
            ...
            cur.execute(
                ...
                INSERT INTO elden.corpus_chunk (
                    id, document_id, entity_type, game_entity_id,
                    name, text, meta, is_dlc, embedding
                )
                VALUES (
                    %s, %s, 'weapon', 'moonlight_greatsword',
            ...
            cur.execute("DROP TABLE IF EXISTS elden.corpus_chunk CASCADE;")
            cur.execute("DROP TABLE IF EXISTS elden.corpus_document CASCADE;")
...
def test_vector_dimension_validation() -> None:
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                "INSERT INTO elden_test.test_vectors (id, embedding) VALUES (%s, %s::vector)",
                (
                    uuid.uuid4(),
                    valid_vector,
                ),
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                INSERT INTO elden_cascade.corpus_document (
                    id, source_type, source_uri
                )
                VALUES (%s, 'test', 'test://cascade')
            """,
                (doc_id,),
            )
            ...
            cur.execute(
                ...
                INSERT INTO elden_cascade.corpus_chunk (
                    id, document_id, entity_type, text, embedding
                )
                VALUES (%s, %s, 'test', 'test text', %s::vector)
            """,
                (chunk_id, doc_id, [0.0] * 1536),
            )
